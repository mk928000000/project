$(function(){
	...
	$(document).on("click",".clk tr", function(){
		/* 초기화 */
			...
			$(this).css({"background-color":"#EBEFE4"})
	...
	$(document).on("click","#paging_button > li", function(){
		...
		$(this).css({"color" : "white","background-color":"#0CDA6D", "border":"1px solid #0CDA6D","border-radius":"10px"})
	})
...
//======================================= update 뷰
	...
	$(document).on("click","#btnEdit",function(){
		...
		if(class_no == '') {
			$(location).attr("href","/login/class.do")
			alert("수정할 강의를 선택하세요")
			return
		}
		var data = {class_no:class_no}
		$.ajax({url:"/login/adminClassEdit.do", data:data,success:function(res){
			console.log(res)
