$(function(){
	...
	//==========tr 선택 이벤트 - 수정/삭제
	$(document).on("click","tr:not(:first-of-type)", function(){
		/* 초기화 */
			$("#aEdit").removeAttr("href")
			$("#aDel").removeAttr("href")
			$("tr").css({"background-color": "white"})
			
			/* 본명령 */
			var classno = $(this).attr("classno")
			var urlEdit = "adminClassEdit.do?classno="+classno
			var urlDel = "adminClassDelete.do?classno="+classno
			$("#aEdit").attr("href",urlEdit)
	...
	var paging = function (onOff, search, option){  // 검색누를때 현재 페이지가 1이 아니면 1페이지부터 안떠서 false 옵션 추가
		...
		$.ajax({url: "/getTotalRecord.do", data:data, success:function(data){
			...
			if(startPage > 1) {
				var li = $("<li></li>").html("<<").attr({"data-page":startPage-1,"id":"before"})
				$("#paging_button").append(li)
			}
			for(var i =startPage; i<=endPage; i++){
				var li = $("<li></li>").html(i).attr("data-page",i)
				$("#paging_button").append(li)
			}
			if(totalPage > endPage) {
				var li = $("<li></li>").html(">>").attr({"data-page":endPage+1,"id":"next"})
				$("#paging_button").append(li)
			}
